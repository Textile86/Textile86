# 👋 Привет, я Егор

🔥 **Начинающий Java-разработчик** в активном поиске коммерческого опыта  
🚀 Цель: расти в backend-разработке и enterprise-решениях

---

### 🧑‍💻 Обо мне

- 💼 Завершаю курс **Хекслет "Java-разработчик"** (модуль 6 из 7)
- 🎯 Осознанно выбрал Java за стабильность, востребованность и мощную экосистему
- 🌱 Изучаю: Java, Spring Boot, JPA, PostgreSQL, Docker
- 📫 Связаться со мной:  
  [![Telegram](https://img.shields.io/badge/-Telegram-090909?style=for-the-badge&logo=telegram&logoColor=27A0D9)](https://t.me/EgorNasonov86) 
  [![Email](https://img.shields.io/badge/-Email-090909?style=for-the-badge&logo=gmail&logoColor=FF5E5E)](mailto:ваш@email.com)

---

### 🛠️ Мой стек

![Java](https://img.shields.io/badge/Java-ED8B00?style=for-the-badge&logo=openjdk&logoColor=white)
![Spring](https://img.shields.io/badge/Spring-6DB33F?style=for-the-badge&logo=spring&logoColor=white)
![PostgreSQL](https://img.shields.io/badge/PostgreSQL-316192?style=for-the-badge&logo=postgresql&logoColor=white)
![JUnit](https://img.shields.io/badge/JUnit-25A162?style=for-the-badge&logo=junit5&logoColor=white)
![Gradle](https://img.shields.io/badge/Gradle-02303A?style=for-the-badge&logo=gradle&logoColor=white)
![GitHub Actions](https://img.shields.io/badge/GitHub_Actions-2088FF?style=for-the-badge&logo=github-actions&logoColor=white)
![Docker](https://img.shields.io/badge/Docker-2496ED?style=for-the-badge&logo=docker&logoColor=white)

---

### 📌 Мои ключевые проекты

#### 🧩 [Игры разума](https://github.com/Textile86/java-project-61)
Консольный набор мини-игр с проверкой логики и математики.  
🔧 **Стек:** Java, Gradle, JUnit, Checkstyle  
✅ CI/CD: GitHub Actions, автоматическая проверка стиля и тестов

#### 🔍 [Вычислитель отличий](https://github.com/Textile86/java-project-71)
Утилита для сравнения файлов (JSON/YAML) с выводом в разных форматах (stylish, plain, json).  
🔧 **Стек:** Java, JCommander, Jackson, JUnit, Gradle

#### ✅ [Валидатор данных](https://github.com/Textile86/java-project-78)
Библиотека для валидации объектов с гибкими правилами (аналог Java Bean Validation).  
🔧 **Стек:** Java, JUnit, Gradle, параметризованные тесты

#### 🌐 [Анализатор страниц](https://github.com/Textile86/java-project-72)
Веб-приложение для SEO-аудита сайтов с базой данных.  
🔧 **Стек:** Java, Javalin, Jsoup, PostgreSQL, JDBC, Thymeleaf  
✅ Деплой: [Railway](https://hexlet-spring-blog-production.up.railway.app/login.html)

#### 📝 [Spring-блог](https://github.com/Textile86/hexlet-spring-blog)
Учебный блог на Spring Boot с CRUD операциями и авторизацией.  
🔧 **Стек:** Spring Boot, Spring MVC, Spring Data JPA, PostgreSQL, Thymeleaf, Bootstrap

#### 📋 [Менеджер задач](https://github.com/Textile86/java-project-99)
Полноценный таск-трекер на Spring Boot с ролевой моделью и фильтрацией.  
🔧 **Стек:** Spring Boot, Spring Security, JPA, PostgreSQL, Liquibase, Thymeleaf, Bootstrap  
🚀 **Демо:** [Render](https://java-project-99-0dzq.onrender.com)  
*(⚠️ Холодный старт до 60 сек. Логин: admin@example.com / qwerty)*

---

### 📚 Курсы и обучение

- 🎓 **Хекслет** — Java-разработчик (модуль 5/7)
- 📘 **Udemy** — Андрей Сумин "Java с нуля до Junior"
- 📱 **Sololearn** — Java Intermediate, Introduction to LLMs, Tech for Everyone

---

### 🎯 Мои цели на ближайшее время

- Завершить обучение на Хекслете
- Написать свой pet-проект с микросервисной архитектурой
- Найти первую коммерческую работу или стажировку
- Углубиться в Docker, Kubernetes и тестирование

---

### 📈 Моя активность

![GitHub stats](https://github-readme-stats.vercel.app/api?username=Textile86&show_icons=true&theme=default&hide=prs,issues)
![Top Langs](https://github-readme-stats.vercel.app/api/top-langs/?username=Textile86&layout=compact&hide=html,css)

---

✨ **Открыт для сотрудничества, консультаций и предложений о работе!**  
📩 Пишите в Telegram или на почту — всегда на связи.
